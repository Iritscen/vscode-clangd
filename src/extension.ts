...
export async function activate(context: vscode.ExtensionContext):
  ...
  context.subscriptions.push(
      // Register for changes in the position of the text cursor in case user
      // asks for "Until Line Change" diagnostics delay
      ...
      vscode.workspace.onDidChangeConfiguration((conf) => {
        ...
        else if (conf.affectsConfiguration(
                     'clangd.diagnosticsDelay.untilLineChange') &&
                 clangdContext)
          clangdContext.diagnosticsWaitForLineChange =
              vscode.workspace.getConfiguration('clangd').get<boolean>(
                  'diagnosticsDelay.untilLineChange') ??
              false;
        else if (conf.affectsConfiguration(
                     'clangd.diagnosticsDelay.untilFileSave') &&
                 clangdContext)
          clangdContext.diagnosticsWaitForFileSave =
              vscode.workspace.getConfiguration('clangd').get<boolean>(
                  'diagnosticsDelay.untilFileSave') ??
              false;
      }));
